...
《原神》（Genshin Impact）多账号任务管理工具，面向同时打理多个账号的玩家和代肝工作室。集中追踪体力、深境螺旋、幻想真境剧诗、危战、剧情任务、限时活动等日常与周期内容，支持桌面端（Windows）与手机端（PWA）。纯本地记录工具，不会连接或操作游戏本体，所有数据仅保存在本机。
...
后端业务逻辑只使用 Python 标准库；应用窗口依赖 `pywebview`（基于系统 WebView2 内核）。开发环境建议使用 Python 3.11 或更高版本。
